...
/*   Updated: 2024/03/07 18:44:46 by baouragh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void hundlle_keys(int *keycode, t_fractal *fractal)
{
     if (*keycode == 53)
        ...
        fractal->x_shift_value -= 0.5 * (fractal->last_zoom);
    else if (*keycode == 124)
        fractal->x_shift_value += 0.5 * (fractal->last_zoom);
    else if (*keycode == 125)
        fractal->y_shift_value -= 0.5 * (fractal->last_zoom);
    else if (*keycode == 126)
        fractal->y_shift_value += 0.5 * (fractal->last_zoom);
    else if (*keycode == 24)
        fractal->max_iter += 10;
...
int     mouse_hook(int button, int x, int y, t_fractal *fractal)
{
        ...
        if (button == 4 || button == 5)
        {
                if (button == 5)
                        ...
                        zoom_factor = 1.1;
                fractal->last_zoom *= zoom_factor;
                mouse_x = (map(x, -2, 2, LENGTH) * fractal->last_zoom) + fractal->x_shift_value;
                mouse_y = (map(y, 2, -2, WIDTH) * fractal->last_zoom) + fractal->y_shift_value;
                ...
                // printf("%f, %f\n",mouse_x - fractal->x_shift_value, mouse_y - fractal->y_shift_value);
                render_fractal(fractal, fractal->argv);
        }
        return (0);
}
